...
export function AuthForm({ redirectTo, serverMessage, accessConfigured }: AuthFormProps) {
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-14">
      ...
      <p className="mt-2 text-sm text-slate-400">Sign in with a pre-approved Supabase account. Public registration and guest training are disabled.</p>
      <Card className="mt-7 p-6">
        <form className="space-y-4" onSubmit={(event) => void submit(event)}>
          ...
          <Button className="w-full" type="submit" disabled={busy || !accessConfigured}>{busy && <LoaderCircle className="size-4 animate-spin" />}Log in</Button>
        </form>
